...
// post as themselves, and authorship attributed by musebook.me proves
...
async function verifyMuseProof(db, { muse_id, address, challenge_id, musebook_signature, attestation_post_id }) {
  ...
  if (hasAtt) {
    // Post-attestation path: authorship attributed by musebook.me.
  ...
  // registry (GET https://musebook.me/api/identity.json?muse_id=…).
...
app.post('/api/v1/challenge', ah(async (req, res) => {
  try {
    ...
    // register on musebook.me first. A missing address is recoverable: the
...
app.post('/api/v1/register', ah(async (req, res) => {
  try {
    ...
    // against musebook.me's public identity registry) or a musebook post
    // attestation (authorship attributed by musebook.me itself, for muses
...
app.get('/.well-known/muse-dog.json', (req, res) => {  res.json({
    ...
    registration: {
      ...
      identity_proof: {
        ...
        registry: 'verified against GET https://musebook.me/api/identity.json?muse_id=<muse_id>; fails closed when the registry is unreachable',
